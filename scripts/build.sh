...
if [ ${#MISSING[@]} -gt 0 ]; then
    ...
    echo "    sudo apt install -y build-essential cmake libsodium-dev nlohmann-json3-dev libgtest-dev"
    echo ""
    echo "  Fedora 安装命令："
...
if ! cmake "$SERVER_DIR" \
    ...
    elif grep -qi "googletest\|gtest" "$CMAKE_ERR_FILE" 2>/dev/null; then
        warn "googletest 下载失败——GitHub 可能不可达"
        warn "安装系统包即可绕过：sudo apt install -y libgtest-dev"
