...
**里程计算规则**：
- 铁路线不是大圆航线——列车沿轨道依次经过中间站，实际走行里程 = 各相邻停站间 Haversine 距离逐段累加
- **直达**：从出发站到到达站之间，该列车停站序列中每一对相邻站的 Haversine 距离求和（含出发站到下一站、…、上一站到到达站）
...
| 页面 | 对应 API | 功能说明 |
|------|----------|----------|
| **查票** | `GET /api/trains/query` | 出发站/到达站下拉选（20站）、日期选择器；结果以卡片列表展示，含车次、时间、余票、票价；直达和换乘分两个标签页 |
