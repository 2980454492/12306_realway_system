...
| 页面 | 对应 API | 功能说明 |
|------|----------|----------|
| **查票** | `GET /api/trains/query` | 出发站/到达站下拉选（20站）、日期选择器；结果以卡片列表展示，含车次、时间、里程、余票、票价；直达和换乘分两个标签页；支持五种排序（发车时间/到达时间/少用时/短距离/低票价，票价按有票席位中最低价排序），默认发车时间优先；支持四种筛选（车型多选/只看有票/出发时间范围/到达时间范围），可同时组合使用 |
